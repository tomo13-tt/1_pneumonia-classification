...
![ResNet18(Frozen)_混同行列](outputs/figures/6_resnet(frozen)_confusion_matrix.png) 
...
![ResNet18(Fine-tuned)_混同行列](outputs/figures/8_resnet(fine-tuned)_confusion_matrix.png)
...
![EfficientNet-B0_混同行列](outputs/figures/10_efficientnet confusion_matrix.png) 
...
次に、誤分類画像についてだが、医療診断におけるX線画像の見方については分からないので、簡単に調べたドメイン知識と絡めながら画像から得られる情報のみで考察をする。人間の肺は正常であればほとんどが空気が占めているので、X線画像で肺は黒く映し出されるようである。しかし、肺炎によって肺に水がたまると、白く濁って映し出されることがあるようだ。  
